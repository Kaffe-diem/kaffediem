defmodule Kaffebase.Accounts do
  ...
  # Old API functions removed - use Phoenix auth functions instead:
  # - create_user → register_user
  # - update_user → update_user_email or update_user_password or update_profile
  # - change_user → change_user_email or change_user_password or change_profile
  ...
  @doc """
  Updates the user profile (name, username, etc).
  """
  ...
  def ensure_admin_user do
    query = from u in User, where: u.is_admin, order_by: [asc: u.inserted_at], limit: 1
  ...
  alias Kaffebase.Accounts.{User, UserToken, UserNotifier}
  ...
  See `Kaffebase.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Kaffebase.Accounts.User.password_changeset/3` for a list of supported options.
